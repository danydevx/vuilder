<template>
  <section
    :id="id"
    class="section section-cards-ribbon py-5 position-relative overflow-hidden"
    :class="schemeClass"
  >
    <div
      v-if="backgroundImg"
    ...
    <div :class="containerClass">
      ...
      <div class="row g-4" :class="justifyClass">
        ...
        >
          <article
            class="card-item h-100 rounded-4 overflow-hidden position-relative"
            :class="{ 'card-item--clickable': card.href || card.onClick }"
            @click="card.onClick ? card.onClick(card) : null"
          >
            <div v-if="card.ribbon" class="card-ribbon" :class="`ribbon-${card.ribbonPosition || 'right'}`">
...
const props = defineProps({
  id: { type: String, default: 'section-cards-ribbon' },
