<template>
  <section
    :id="id"
    class="section section-videos-minimal py-5 position-relative overflow-hidden"
    :class="schemeClass"
  >
    <div
      v-if="backgroundImg"
    ...
    <div :class="containerClass">
      ...
      <div class="row g-4" :class="justifyClass">
        ...
        >
          <article
            class="video-card text-center"
            @click="openModal(video)"
          >
            <figure class="video-thumb rounded-4 overflow-hidden position-relative mb-3">
              <img
              ...
              <span v-if="video.duration" class="duration-badge position-absolute bottom-0 end-0 m-2">
                {{ video.duration }}
              </span>
            </figure>

            <h3 v-if="video.title" class="video-title h6 fw-semibold mb-1">{{ video.title }}</h3>
            <p v-if="video.category || video.year" class="video-meta  small mb-0">
              <span v-if="video.category">{{ video.category }}</span>
              <span v-if="video.category && video.year"> · </span>
...
const props = defineProps({
  id: { type: String, default: 'section-videos-minimal' },
...
const justifyClass = computed(() => {
  ...
  return map[props.justify] || map.center
})

const colClass = computed(() => {
  switch (props.cols) {
    case 2: return 'col-6 col-md-3'
    case 3: return 'col-6 col-md-4'
    case 4: return 'col-6 col-lg-3'
    case 5: return 'col-4 col-md-2'
    default: return 'col-6 col-lg-3'
  }
})
